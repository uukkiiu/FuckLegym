...
import legym.fuck.logic.legym.network.model.huodong.isRegisterAble
 ...
 *
 */
class AutoRegisterFragment : BaseBindingBottomSheetFragment<FragmentAutoRegisterBinding>() {
    ...
    override fun FragmentAutoRegisterBinding.initViews() {
        title.setText(R.string.activity_quickly_register)
    ...
    override fun FragmentAutoRegisterBinding.initData() {
        ...
        selectedActivities.retainAll { it.isRegisterAble }
        viewModel.validActivities = selectedActivities
        if (viewModel.validActivities.isEmpty()) {
            ToastUtil.info("当前选中的还能报名的活动为0")
            dismiss()
        } else viewModel.doRegister()
    }
